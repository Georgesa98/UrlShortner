  ...
  try {
    ...
    if (e instanceof AxiosError) {
      return {
        success: false,
        status: e.response?.status || 500,
        message: e.response?.data?.message || "Failed to bulk delete users",
      };
    }
...
export async function toggleBanUserAction({ user_id }: { user_id: number }) {
  try {
    const response = await axiosInstance.patch(`/admin/user/${user_id}/ban/`);
    return {
      success: true,
      status: response.status,
      message: response.data.message,
    ...
    if (e instanceof AxiosError) {
      return {
        success: false,
        status: e.response?.status || 500,
        message: e.response?.data?.message || "Failed to toggle ban user",
      };
    }
    return {
      success: false,
...
export async function getUserUrlsAction({
  ...
  try {
    const response = await axiosInstance.get(
      `/admin/url/user/${user_id}/?page=${page}&limit=${limit}`,
    );
    return {
      success: true,
      status: response.status,
...
export async function getUserDetailsAction({ user_id }: { user_id: string }) {
  try {
    const response = await axiosInstance.get(`/admin/user/details/${user_id}/`);
    return {
      success: true,
      status: response.status,
      data: response.data.data,
